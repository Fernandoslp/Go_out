let dados= [
    {
        titulo : "Varandeck",
        descricao : "Local com ótimos valores, tem a area externa e interna, com boa variedade de bebidas e costuma lotar aos finais de semana. Nota 4.6 no google",
        local:"Endereço:Rua Irineu Bonardi, 173 - Vila Pedroso, São Paulo - SP, 08011-180",
        link: "https://www.instagram.com/varandeck/",
        imagem:"imagens/ftvarandeck.png",
        tags: ['Lounge Bar', 'Área externa', 'Entrada paga','Mais baratos','Sao miguel'],
        id: 1
     },
     {
        titulo : "Nakka Bar",
        descricao : "Local com boas bebidas, atendentes com bom humor e muito frequentado por quem mora em são miguel. Nota 4.4 no google",
        local:"Endereço:Rua Vilma, 51 - São Miguel Paulista, São Paulo - SP, 08060-090",
        link: "https://www.instagram.com/nakkabar?igsh=MWNwY2N2dzVmb2Jjdg==",
        imagem:"imagens/ftnakkabar.png",
        tags: ['Lounge Bar', 'Entrada paga','Área externa','Sao miguel'],
        id: 2
     },
     {
        titulo : "Itaquera beach bar",
        descricao : "Local bem arejado, tem a opção de poder usar as quadras para esportes com areia",
        local:"Endereço:Rua São Teodoro, 193 - Itaquera, São Paulo - SP, 08290-005",
        link: "https://www.instagram.com/itaquerabeachbar?igsh=MWx3bHRwemxkZjB2Nw==",
        imagem: "imagens/ftitaquerabeachbar.jpg",
        tags: ['Lounge Bar', 'Área externa','Mais baratos','Itaquera'],
        id: 3
     },
     {
        titulo : "Amém lounge Store",
        descricao : "Local bem completo, djs muito bons, sempre lotado após as 23h",
        local:"Endereço:Rua Francisco Alarico Bérgamo, 80 - Vila Taquari, São Paulo - SP, 08230-010",
        link: "https://www.instagram.com/amemloungestore?igsh=MWFseGxyNHU3Zzhlaw==",
        imagem:"imagens/ftamem.png",
        tags: ['Lounge Bar','Entrada paga','Itaquera'],
        id: 4
     },
     {
      titulo : "Nakka Club",
      descricao : "Local costuma ter djs diferentes tocando e além de ter variedade de bebidas tem um local fechado que é ideal em dias frios, dentro do local antes de entrar no salão princial a varias produtos a venda para fumantes",
      local:"Endereço:Av. Amador Bueno da Veiga, 751 - Vila Lais, São Paulo - SP, 03635-000",
      link: "https://www.instagram.com/nakkaclub/p/CzJkp9WrtzD",
      imagem: "imagens/ftnakkaclub.png",
      tags: ['Lounge Bar', 'Entrada paga'],
      id: 5
   },
   {
      titulo : "Bar Pitangueira",
      descricao : "Local aberto com tendas para cobrir algumas partes e cadeiras de praia, uma parte interna e eventos no local ",
      local:"Endereço:R. Siqueira Campos, 252 - Centro de Guarulhos, Guarulhos - SP, 07110-110",
      link: "https://www.instagram.com/barpitangueira",
      imagem: "imagens/ftbarpitangueira.jpg",
      tags: ['Lounge Bar','Entrada paga','Área externa','Guarulhos'],
      id: 6
   },
   {
      titulo : "Bar do Juiz - Patriarca",
      descricao : "Local com parte interna e externa com bastante espaço com show de famosos e sempre lotado",
      local:"Endereço:Av. Antonio Estevão de Carvalho 2100, São Paulo, SP, 03540-200",
      link: "https://www.instagram.com/bardojuiz",
      imagem: "imagens/ftbardojuizpatriarca.jpg",
      tags: ['Lounge Bar','Entrada paga','Patriarca'],
      id: 7
   },
   {
      titulo : "Bar do Juiz - Imperador",
      descricao : "Local com parte interna e externa com bastante espaço com show de famosos e variedade de drinks",
      local:"Endereço:Rua José Garcia Terra 285, São Paulo, SP, 03675-000",
      link: "https://www.instagram.com/bardojuiz",
      imagem: "imagens/ftbardojuizimperador.jpg",
      tags: ['Lounge Bar','Entrada paga','Patriarca'],
      id: 8
   },
   {
      titulo : "Drinkeria Lux",
      descricao : "Local com parte interna e algumas mesas na calçada, drinks variados e projetor que passa o jogos de futebol geralmente",
      local:"Endereço:R. Tiago Ferreira, 390 - Vila Pedroso, São Paulo - SP, 08011-270",
      link: "https://www.instagram.com/drinkeria.lux",
      imagem: "imagens/ftdrinkerialux.jpg",
      tags: ['Lounge Bar','Entrada gratis','Sao miguel'],
      id: 9
   },
   {
      titulo : "The Place Drinks",
      descricao : `Local com parte interna e externa com bastante espaço, drinks variados e sempre lotado 
      com eventos variados que pode podem ser conferidos pelas redes socias do local`,
      local:"Endereço:Av. São Miguel 5057 (Parque Boturussu), São Paulo, SP ",
      link: "https://www.instagram.com/theplacedrinks?igsh=aHU3bmx6c2ZpMW0w",
      imagem: "imagens/fttheplacedrinks.jpg",
      tags: ['Lounge Bar','Entrada paga','Parque Boturusso'],
      id: 10
   },
   {
      titulo : "Prainha Lounge Bar",
      descricao : "Local com parte interna com bastante espaço e um leve canto externo, drinks variados e ótimos preços",
      local:"Endereço:R. Amorim Diniz, 200 - Vila Sao Geraldo, São Paulo - SP, 03730-040",
      link: "https://www.instagram.com/prainhaloungebar?igsh=djNsOHhhYTVuMWtn",
      imagem: "imagens/ftprainhaloungebar.jpg",
      tags: ['Lounge Bar','Entrada paga','Mais baratos','Penha'],
      id: 11
   },
   {
      titulo : "Dos Crias Bar",
      descricao : "Local agradável com bons preços parte interna mais algumas mesas na calçada do local",
      local:"Endereço:R. Vilma, 145 - Vila Jacuí, São Paulo - SP, 08060-090 ",
      link: "https://www.instagram.com/doscriasbar?igsh=YzduMXB2NWpyc2Y1",
      imagem: "imagens/ftdoscriasbar.jpg",
      tags: ['Lounge Bar','Entrada gratis','Mais baratos','Sao miguel'],
      id: 12
   },

 ];